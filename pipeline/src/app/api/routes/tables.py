...
# Assumes schema name and table name are the same



def get_schema_metadata(schema: pa.DataFrameModel) -> TableSchema:
    ...
    with get_connection() as conn:
        table_name = to_snake_case(schema.__name__)
        try:
...
def list_all_tables():
    ...
    for asset_def in defs.assets:
        ...
        if "gold" in medallions:
            table_names.add(asset_def.key.to_user_string())
...
def get_table(
    ...
    for asset_def in defs.assets:
        ...
        if "gold" in medallions:
            table_names.add(asset_def.key.to_user_string())
    # TODO: In the future use dagster-pandera package to combine assets with schemas.
    ...
    if not table_name in table_names:
        raise HTTPException(
            404, 
            f"Table not found. Must be one of {table_names}",
        )
    
    if not table_name in schema_map.keys():
        schema_list = '\n'.join([schema.__name__ for schema in schemas])
        raise HTTPException(
            404,
            f"No schema found for {table_name}. Available schemas:\n{schema_list}"
        )
    
    try:
        schema_metadata = get_schema_metadata(schema_map[table_name])
